...
export default class CodePin extends Component {
  ...
  renderPrivateCode = (Size) => {
    ...
    for(let i = 0; i < Size; i++)
      arrayCode.push(<Animated.View  style={[styles.circleStyle,{backgroundColor:this.props.PinColor, marginBottom: this.Animation[i].margin }]} key={i}/>);
    return arrayCode;
  }
  ...
  renderPrivateCodeAnimation = (Size) => {
    ...
    let arrayCode = [];
    for(let i = 0; i < Size; i++)
      arrayCode.push(<Animated.View key={i} style={[styles.circleColor,{backgroundColor: this.props.CodeColor, marginBottom: this.Animation[i].margin, opacity: this.interpolate[i].opacity, zIndex: zIndex-- ,width: this.interpolate[i].width, left: this.Animation[i].position}]}/>)
    return arrayCode
  }
      ...
      return (
        <TouchableOpacity key={index} onPress={() => { this.code.pop(); this.rendAnimate() }}>
          <View style={
            ...
            <Icon
              name={'md-backspace'}
              style={[{color:props.TextColor, fontSize: 30}]}
            />
          </View>
        </TouchableOpacity>
      )
