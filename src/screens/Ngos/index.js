...
class NgosScreen extends Component {
  ...
  handleNgoPress(name, logo, sharedImageId) {
    const options = {
      ...
      customTransition: {
        animations: [{ type: 'sharedElement', fromId: sharedImageId, toId: sharedImageId, startDelay: 0, springVelocity: 0.2, duration: 0.5 }],
        duration: 0.8
      }
    }
        ...
        <View style={styles.bodyContainer}>
          <View style={styles.ngoWrapper}>
            {ngos.map((ngo, index) => {
              ...
              return (
                <TouchableHighlight key={index} underlayColor="transparent" onPress={() => this.handleNgoPress(ngoName, ngoLogo, sharedElementId)}>
                  <View style={styles.imageWrapper}>
                    <Navigation.Element style={styles.shareElement} elementId={sharedElementId}>
                      <FastImage resizeMode="contain" style={styles.logo} source={{ uri: ngoLogo }} />
                    </Navigation.Element>
                  </View>
                </TouchableHighlight>
              )
